...
public class SecondActivity_ldpi extends AppCompatActivity {

    int x2,y2 = 450,x4,x5 = -720,falling_speed = 0;
    int x3 = 0,y4,y5,y6 = -525,y7,count = 0,press_count,index = 0;
    int i = 0,j = 0,index1 = 0,k = 0,l = 1,m = 9,n = 0,p = 0;
    ...
    class BallView extends View {
        ...
        public void onDraw(Canvas canvas) {
            ...
            if( starting ) {
                ...
                menu_x = canw_mid-75;
                menu_y = canh_mid+101;
                circle_x = canw_mid+41;
                circle_y = canh_mid+101;

                ready_x = (canw_mid/2)-20;
            ...
            if( initial ) {
                ...
                canvas.drawBitmap(tap, x2-70+15, y2+120-66, null);
                if ( tapping_index == 10 ) {
                    if ( tap_index < 2 )
                        tap_index++;
                    else
                ...
                if( finger_flag ) {
                    if (finger_index >= 50) {
                        ...
                        canvas.drawBitmap(finger[0], (x2+68)-8 - finger_index, (y2+251)-88 - finger_index, null);
                }
                else {
                    if( wait>0 ) {
                        canvas.drawBitmap(finger[1], (x2+15)-8, y2+186-88, null);
                        finger_index--;
                    }
                    else {
                        canvas.drawBitmap(finger[0], (x2+18)-8 + finger_index, (y2+201)-88 + finger_index, null);
                        if ( finger_index >= 50 ) {
                            finger_flag = true;
                            finger_index = 0;
                        }
                ...
                if ( !paused && is_alive ) {
                    ...
                    else if ( x5 < 0 && x4 < (-up_pipe.getWidth()) ) {
                    ...
                    if ( flag_point_x4 && x4 < 110 ) {
                        point.start();
                        flag_point_x4 = false;
                        point_flag = true;
                    }
                    else if ( flag_point_x5 && x5 < 110 ) {
                        point.start();
                        flag_point_x5 = false;
                        point_flag = true;
                    }
                ...
                if ( !paused && is_alive ) {
                    if (k < 10) {
                        ...
                        canvas.drawBitmap(points[l], canw_mid-9, 25, null);
                        canvas.drawBitmap(points[m], canw_mid+9, 25, null);
                    } else if (k < 1000) {
                        canvas.drawBitmap(points[l], canw_mid-19, 25, null);
                        canvas.drawBitmap(points[m], canw_mid, 25, null);
                        canvas.drawBitmap(points[n], canw_mid+19, 25, null);
                    }else{
                        canvas.drawBitmap(points[l], canw_mid-26, 25, null);
                        canvas.drawBitmap(points[m], canw_mid-9, 25, null);
                        canvas.drawBitmap(points[n], canw_mid+9, 25, null);
                        canvas.drawBitmap(points[p], canw_mid+25, 25, null);
                    }
            ...
            if ( ( !paused  && is_alive ) || initial ) {
                ...
                if( k<=k_old && !new_best_flag )
                    canvas.drawBitmap(best, canw_mid-75, canh_mid-39, null);
                else {
                    canvas.drawBitmap(new_best, canw_mid-75, canh_mid-39, null);
                    if (new_index == 5) {
                        if (new_wait <= 5) {
                            canvas.drawBitmap(new_show, canw_mid-75, canh_mid-39, null);
                            new_wait++;
                        } else {
                            new_index = 0;
                            new_wait = 0;
                ...
                if( is_win )
                    canvas.drawBitmap(you_win, canw_mid-95, canh_mid-150, null);
                else if( !is_alive )
                    canvas.drawBitmap(game_over, canw_mid-85, canh_mid-100, null);

                if ( k < 10 )
                    canvas.drawBitmap(points[k], canw_mid-8, canh_mid-10, null);
                else if ( k < 100 ) {
                    canvas.drawBitmap(points[l], canw_mid-17, canh_mid-10, null);
                    canvas.drawBitmap(points[m], canw_mid, canh_mid-10, null);
                } else if ( k < 1000 ) {
                    canvas.drawBitmap(points[l], canw_mid-25, canh_mid-10, null);
                    canvas.drawBitmap(points[m], canw_mid-8, canh_mid-10, null);
                    canvas.drawBitmap(points[n], canw_mid+9, canh_mid-10, null);
                }else{
                    canvas.drawBitmap(points[l], canw_mid-34, canh_mid-10, null);
                    canvas.drawBitmap(points[m], canw_mid-17, canh_mid-10, null);
                    canvas.drawBitmap(points[n], canw_mid, canh_mid-10, null);
                    canvas.drawBitmap(points[p], canw_mid+17, canh_mid-10, null);
                }

                if ( k_old < 10 )
                    canvas.drawBitmap(points[k_old], canw_mid-8, canh_mid+62, null);
                else if ( k_old < 100 ) {
                    canvas.drawBitmap(points[l_old], canw_mid-17, canh_mid+62, null);
                    canvas.drawBitmap(points[m_old], canw_mid, canh_mid+62, null);
                } else if ( k_old < 1000 ) {
                    canvas.drawBitmap(points[l_old], canw_mid-25, canh_mid+62, null);
                    canvas.drawBitmap(points[m_old], canw_mid-8, canh_mid+62, null);
                    canvas.drawBitmap(points[n_old], canw_mid+9, canh_mid+62, null);
                }else{
                    canvas.drawBitmap(points[l_old], canw_mid-34, canh_mid+62, null);
                    canvas.drawBitmap(points[m_old], canw_mid-17, canh_mid+62, null);
                    canvas.drawBitmap(points[n_old], canw_mid, canh_mid+62, null);
                    canvas.drawBitmap(points[p_old], canw_mid+17, canh_mid+62, null);
                }
            }
            if ((((x4 < (x2 + 27))&& ((x4 + up_pipe.getWidth()) > x2) && (y2 < (y4 + up_pipe.getHeight() - 14))) ||
                    ((x5 < (x2 + 27)) && ((x5 + up_pipe.getWidth()) > x2) && (y2 < (y6 + up_pipe.getHeight() - 14))) ||
                    ((x4 < (x2 + 27)) && (((x4 + down_pipe.getWidth()) - 10) > x2) && ((y2 + 23) > y5)) ||
                    ((x5 < (x2 + 27)) && (((x5 + down_pipe.getWidth()) - 10) > x2) && ((y2 + 23) > y7)) ||
                    (y2 >= (canh-37))) && is_alive ) {
                hit.start();
                die.start();
                is_alive = false;
    ...
    public boolean onTouchEvent(MotionEvent event){
        ...
        if (x == MotionEvent.ACTION_DOWN) {
            if( initial )
                initial = false;
            else if ( (pressX < 38 && pressY < 38) && !paused ) {
                button.start();
                paused = true;
            }
            else if ( paused && pressX>circle_x && pressX<(circle_x+33) && pressY>circle_y && pressY<(circle_y+33) ){
                button.start();
                paused = false;
            }
            else if ( !is_alive && pressX>circle_x && pressX<(circle_x+33) && pressY>circle_y && pressY<(circle_y+33) ){
            ...
            else if( (!is_alive || paused ) && pressX>menu_x && pressX<(menu_x+75) && pressY>menu_y && pressY<(menu_y+38) ){
