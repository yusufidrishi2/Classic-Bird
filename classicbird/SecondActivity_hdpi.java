...
public class SecondActivity_hdpi extends AppCompatActivity {

    int x2,y2 = 450,x4,x5 = -720,falling_speed = 0;
    int x3 = 0,y4,y5,y6 = -525,y7,count = 0,press_count,index = 0;
    int i = 0,j = 0,index1 = 0,k = 0,l = 0,m = 0,n = 0,p = 0;
    ...
    boolean flag = false,flag_point_x4 = true,flag_point_x5,new_activity = false,initial = true;
    ...
    class BallView extends View {
        ...
        public void onDraw(Canvas canvas) {
            ...
            if( starting ) {
                ...
                canh = canvas.getHeight() - (canvas.getHeight() / 8)+50;
                ...
                menu_x = canw_mid-139;
                menu_y = canh_mid+114;
                circle_x = canw_mid+94;
                circle_y = canh_mid+114;

                ready_x = canw_mid/2-44;
            ...
            if( initial ) {
                ...
                canvas.drawBitmap(tap, x2-98, y2+120, null);
                if ( tapping_index == 10 ) {
                    if ( tap_index < 2 )
                        tap_index++;
                    else
                ...
                if( finger_flag ) {
                    if (finger_index >= 50) {
                        ...
                        canvas.drawBitmap(finger[0], (x2+86) - finger_index, (y2+291) - finger_index, null);
                }
                else {
                    if( wait>0 ) {
                        canvas.drawBitmap(finger[1], (x2+31), y2+201, null);
                        finger_index--;
                    }
                    else {
                        canvas.drawBitmap(finger[0], (x2+36) + finger_index, (y2+241) + finger_index, null);
                        if ( finger_index >= 50 ) {
                            finger_flag = true;
                            finger_index = 0;
                        }
                ...
                if ( !paused && is_alive ) {
                    ...
                    else if ( x5 < 50 && x4 < (-up_pipe.getWidth()) ) {
                    ...
                    if ( flag_point_x4 && x4 < 200 ) {
                        point.start();
                        flag_point_x4 = false;
                        point_flag = true;
                    }
                    else if ( flag_point_x5 && x5 < 200 ) {
                        point.start();
                        flag_point_x5 = false;
                        point_flag = true;
                    }
                ...
                if ( !paused && is_alive ) {
                    if (k < 10) {
                        ...
                        canvas.drawBitmap(points[l], canw_mid - 18, 25, null);
                        canvas.drawBitmap(points[m], canw_mid + 18, 25, null);
                    } else if (k < 1000) {
                        canvas.drawBitmap(points[l], canw_mid - 36, 25, null);
                        canvas.drawBitmap(points[m], canw_mid, 25, null);
                        canvas.drawBitmap(points[n], canw_mid + 36, 25, null);
                    }else{
                        canvas.drawBitmap(points[l], canw_mid -52, 25, null);
                        canvas.drawBitmap(points[m], canw_mid -18, 25, null);
                        canvas.drawBitmap(points[n], canw_mid +18, 25, null);
                        canvas.drawBitmap(points[p], canw_mid +52, 25, null);
                    }
            ...
            if ( ( !paused  && is_alive ) || initial ) {
                ...
                if( k<=k_old && !new_best_flag )
                    canvas.drawBitmap(best, canw_mid-139, canh_mid-169, null);
                else {
                    canvas.drawBitmap(new_best, canw_mid-139, canh_mid-169, null);
                    if (new_index == 5) {
                        if (new_wait <= 5) {
                            canvas.drawBitmap(new_show, canw_mid-139, canh_mid-169, null);
                            new_wait++;
                        } else {
                            new_index = 0;
                            new_wait = 0;
                ...
                if( is_win )
                    canvas.drawBitmap(you_win, canw_mid-189, canh_mid-329, null);
                else if( !is_alive )
                    canvas.drawBitmap(game_over, canw_mid-179, canh_mid-292, null);

                if ( k < 10 )
                    canvas.drawBitmap(points[k], canw_mid-8, canh_mid-110, null);
                else if ( k < 100 ) {
                    canvas.drawBitmap(points[l], canw_mid-16, canh_mid-110, null);
                    canvas.drawBitmap(points[m], canw_mid+19, canh_mid-110, null);
                } else if ( k < 1000 ) {
                    canvas.drawBitmap(points[l], canw_mid-42, canh_mid-110, null);
                    canvas.drawBitmap(points[m], canw_mid-8, canh_mid-110, null);
                    canvas.drawBitmap(points[n], canw_mid+26, canh_mid-110, null);
                }else{
                    canvas.drawBitmap(points[l], canw_mid-50, canh_mid-110, null);
                    canvas.drawBitmap(points[m], canw_mid-16, canh_mid-110, null);
                    canvas.drawBitmap(points[n], canw_mid+19, canh_mid-110, null);
                    canvas.drawBitmap(points[p], canw_mid+54, canh_mid-110, null);
                }

                if ( k_old < 10 )
                    canvas.drawBitmap(points[k_old], canw_mid-8, canh_mid+33, null);
                else if ( k_old < 100 ) {
                    canvas.drawBitmap(points[l_old], canw_mid-16, canh_mid+33, null);
                    canvas.drawBitmap(points[m_old], canw_mid+19, canh_mid+33, null);
                } else if ( k_old < 1000 ) {
                    canvas.drawBitmap(points[l_old], canw_mid-42, canh_mid+33, null);
                    canvas.drawBitmap(points[m_old], canw_mid-8, canh_mid+33, null);
                    canvas.drawBitmap(points[n_old], canw_mid+26, canh_mid+33, null);
                }else{
                    canvas.drawBitmap(points[l_old], canw_mid-50, canh_mid+33, null);
                    canvas.drawBitmap(points[m_old], canw_mid-16, canh_mid+33, null);
                    canvas.drawBitmap(points[n_old], canw_mid+19, canh_mid+33, null);
                    canvas.drawBitmap(points[p_old], canw_mid+54, canh_mid+33, null);
                }
            }
            if ((((x4 < (x2 + 56)) && ((x4 + up_pipe.getWidth()) > x2) && (y2 < (y4 + up_pipe.getHeight() - 14))) ||
                    ((x5 < (x2 + 56)) && ((x5 + up_pipe.getWidth()) > x2) && (y2 < (y6 + up_pipe.getHeight() - 14))) ||
                    ((x4 < (x2 + 56)) && (((x4 + down_pipe.getWidth()) - 50) > x2) && ((y2 + 46) > y5)) ||
                    ((x5 < (x2 + 56)) && (((x5 + down_pipe.getWidth()) - 50) > x2) && ((y2 + 46) > y7)) ||
                    (y2 >= (canh-70))) && is_alive ) {
                hit.start();
                die.start();
                is_alive = false;
    ...
    public boolean onTouchEvent(MotionEvent event){
        ...
        if (x == MotionEvent.ACTION_DOWN) {
            if( initial )
                initial = false;
            else if ( (pressX < 67 && pressY < 64) && !paused ) {
                button.start();
                paused = true;
            }
            else if ( paused && pressX>circle_x && pressX<(circle_x+64) && pressY>circle_y && pressY<(circle_y+58) ){
                button.start();
                paused = false;
            }
            else if ( !is_alive && pressX>circle_x && pressX<(circle_x+64) && pressY>circle_y && pressY<(circle_y+84) ){
                ...
                flag = false; flag_point_x4 = true; new_activity = false; initial = true;
            ...
            else if( (!is_alive || paused ) && pressX>menu_x && pressX<(menu_x+150) && pressY>menu_y && pressY<(menu_y+75) ){
            ...
            //Toast.makeText(this, "y4 = "+y4+" y6 = "+y6, Toast.LENGTH_LONG).show();
