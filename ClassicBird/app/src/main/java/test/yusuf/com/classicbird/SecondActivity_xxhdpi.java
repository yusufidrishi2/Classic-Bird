...
public class SecondActivity_xxhdpi extends AppCompatActivity {
    ...
    class BallView extends View {
        ...
        public void onDraw(Canvas canvas) {
            ...
            if( starting ) {
                ...
                menu_x = canw_mid-266;
                menu_y = canh_mid+330;
                circle_x = canw_mid+194;
                circle_y = canh_mid+330;
            ...
            if( initial ) {
                ...
                canvas.drawBitmap(tap, x2-206, y2+220, null);
                if ( tapping_index == 10 ) {
                    if ( tap_index < 2 )
                        tap_index++;
                    else
                ...
                if( finger_flag ) {
                    if (finger_index >= 50) {
                        ...
                        canvas.drawBitmap(finger[0], (x2+102) - finger_index, (y2+503) - finger_index, null);
                }
                else {
                    if( wait>0 ) {
                        canvas.drawBitmap(finger[1], (x2+47), y2+503, null);
                        finger_index--;
                    }
                    else {
                        canvas.drawBitmap(finger[0], (x2+52) + finger_index, (y2+435) + finger_index, null);
                        if ( finger_index >= 50 ) {
                            finger_flag = true;
                            finger_index = 0;
                        }
                ...
                if ( !paused && is_alive ) {
                    ...
                    if ( flag_point_x4 && x4 < 490 ) {
                        point.start();
                        flag_point_x4 = false;
                        point_flag = true;
                    }
                    else if ( flag_point_x5 && x5 < 490 ) {
                        point.start();
                        flag_point_x5 = false;
                        point_flag = true;
                    }
                ...
                if ( !paused && is_alive ) {
                    if (k < 10) {
                        ...
                        canvas.drawBitmap(points[l], canw_mid-41, 25, null);
                        canvas.drawBitmap(points[m], canw_mid+29, 25, null);
                    } else if (k < 1000) {
                        canvas.drawBitmap(points[l], canw_mid - 70, 25, null);
                        canvas.drawBitmap(points[m], canw_mid, 25, null);
                        canvas.drawBitmap(points[n], canw_mid+70, 25, null);
                    }else{
                        canvas.drawBitmap(points[l], canw_mid-110, 25, null);
                        canvas.drawBitmap(points[m], canw_mid-41, 25, null);
                        canvas.drawBitmap(points[n], canw_mid+29, 25, null);
                        canvas.drawBitmap(points[p], canw_mid+98, 25, null);
                    }
            ...
            if ( ( !paused  && is_alive ) || initial ) {
                ...
                if( k<=k_old && !new_best_flag )
                    canvas.drawBitmap(best, canw_mid-268, canh_mid-234, null);
                else {
                    canvas.drawBitmap(new_best, canw_mid-268, canh_mid-234, null);
                    if (new_index == 5) {
                        if (new_wait <= 5) {
                            canvas.drawBitmap(new_show, canw_mid-268, canh_mid-234, null);
                            new_wait++;
                        } else {
                            new_index = 0;
                            new_wait = 0;
                ...
                if( is_win )
                    canvas.drawBitmap(you_win, canw_mid-350, canh_mid-497, null);
                else if( !is_alive )
                    canvas.drawBitmap(game_over, canw_mid-340, canh_mid-460, null);

                if ( k < 10 )
                    canvas.drawBitmap(points[k], canw_mid-8, canh_mid-108, null);
                else if ( k < 100 ) {
                    canvas.drawBitmap(points[l], canw_mid-33, canh_mid-115, null);
                    canvas.drawBitmap(points[m], canw_mid+36, canh_mid-115, null);
                } else if ( k < 1000 ) {
                    canvas.drawBitmap(points[l], canw_mid-79, canh_mid-115, null);
                    canvas.drawBitmap(points[m], canw_mid-8, canh_mid-115, null);
                    canvas.drawBitmap(points[n], canw_mid+62, canh_mid-115, null);
                }else{
                    canvas.drawBitmap(points[l], canw_mid-102, canh_mid-115, null);
                    canvas.drawBitmap(points[m], canw_mid-33, canh_mid-115, null);
                    canvas.drawBitmap(points[n], canw_mid+36, canh_mid-115, null);
                    canvas.drawBitmap(points[p], canw_mid+106, canh_mid-115, null);
                }

                if ( k_old < 10 )
                    canvas.drawBitmap(points[k_old], canw_mid-8, canh_mid+172, null);
                else if ( k_old < 100 ) {
                    canvas.drawBitmap(points[l_old], canw_mid-33, canh_mid+172, null);
                    canvas.drawBitmap(points[m_old], canw_mid+36, canh_mid+172, null);
                } else if ( k_old < 1000 ) {
                    canvas.drawBitmap(points[l_old], canw_mid-79, canh_mid+172, null);
                    canvas.drawBitmap(points[m_old], canw_mid-8, canh_mid+172, null);
                    canvas.drawBitmap(points[n_old], canw_mid+62, canh_mid+172, null);
                }else{
                    canvas.drawBitmap(points[l_old], canw_mid-102, canh_mid+172, null);
                    canvas.drawBitmap(points[m_old], canw_mid-33, canh_mid+172, null);
                    canvas.drawBitmap(points[n_old], canw_mid+36, canh_mid+172, null);
                    canvas.drawBitmap(points[p_old], canw_mid+106, canh_mid+172, null);
                }
            }
            if ((((x4 < (x2 + 110)) && ((x4 + up_pipe.getWidth()) > x2) && (y2 < (y4 + up_pipe.getHeight() - 14))) ||
                    ((x5 < (x2 + 110)) && ((x5 + up_pipe.getWidth()) > x2) && (y2 < (y6 + up_pipe.getHeight() - 14))) ||
                    ((x4 < (x2 + 110)) && (((x4 + down_pipe.getWidth()) - 50) > x2) && ((y2 + 60) > y5)) ||
                    ((x5 < (x2 + 110)) && (((x5 + down_pipe.getWidth()) - 50) > x2) && ((y2 + 60) > y7)) ||
                    (y2 >= (canh-150))) && is_alive ) {
                hit.start();
                die.start();
                is_alive = false;
    ...
    public boolean onTouchEvent(MotionEvent event){
        ...
        if (x == MotionEvent.ACTION_DOWN) {
            if( initial )
                initial = false;
            else if ( (pressX < 132 && pressY < 130) && !paused ) {
                button.start();
                paused = true;
            }
            else if ( paused && pressX>circle_x && pressX<(circle_x+128) && pressY>circle_y && pressY<(circle_y+120) ){
                button.start();
                paused = false;
            }
            else if ( !is_alive && pressX>circle_x && pressX<(circle_x+128) && pressY>circle_y && pressY<(circle_y+120) ){
            ...
            else if( (!is_alive || paused ) && pressX>menu_x && pressX<(menu_x+293) && pressY>menu_y && pressY<(menu_y+152) ){
