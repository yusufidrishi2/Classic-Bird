...
public class SecondActivity_mdpi extends AppCompatActivity {
    ...
    class BallView extends View {
        ...
        public void onDraw(Canvas canvas) {
            ...
            if( starting ) {
                ...
                menu_x = canw_mid-97;
                menu_y = canh_mid+39;
                circle_x = canw_mid+56;
                circle_y = canh_mid+39;

                ready_x = canw_mid/2+10;
            ...
            if( initial ) {
                ...
                canvas.drawBitmap(tap, x2-70, y2+120, null);
                if ( tapping_index == 10 ) {
                    if ( tap_index < 2 )
                        tap_index++;
                    else
                ...
                if( finger_flag ) {
                    if (finger_index >= 50) {
                        ...
                        canvas.drawBitmap(finger[0], (x2+68) - finger_index, (y2+251) - finger_index, null);
                }
                else {
                    if( wait>0 ) {
                        canvas.drawBitmap(finger[1], (x2+15), y2+186, null);
                        finger_index--;
                    }
                    else {
                        canvas.drawBitmap(finger[0], (x2+18) + finger_index, (y2+201) + finger_index, null);
                        if ( finger_index >= 50 ) {
                            finger_flag = true;
                            finger_index = 0;
                        }
                ...
                if ( !paused && is_alive ) {
                    ...
                    if ( flag_point_x4 && x4 < 210 ) {
                        point.start();
                        flag_point_x4 = false;
                        point_flag = true;
                    }
                    else if ( flag_point_x5 && x5 < 210 ) {
                        point.start();
                        flag_point_x5 = false;
                        point_flag = true;
                    }
                ...
                if ( !paused && is_alive ) {
                    if (k < 10) {
                        ...
                        canvas.drawBitmap(points[l], canw_mid-12, 25, null);
                        canvas.drawBitmap(points[m], canw_mid+12, 25, null);
                    } else if (k < 1000) {
                        canvas.drawBitmap(points[l], canw_mid-27, 25, null);
                        canvas.drawBitmap(points[m], canw_mid, 25, null);
                        canvas.drawBitmap(points[n], canw_mid+26, 25, null);
                    }else{
                        canvas.drawBitmap(points[l], canw_mid-36, 25, null);
                        canvas.drawBitmap(points[m], canw_mid-12, 25, null);
                        canvas.drawBitmap(points[n], canw_mid+12, 25, null);
                        canvas.drawBitmap(points[p], canw_mid+35, 25, null);
                    }
            ...
            if ( ( !paused  && is_alive ) || initial ) {
                ...
                if( k<=k_old && !new_best_flag )
                    canvas.drawBitmap(best, canw_mid-97, canh_mid-149, null);
                else {
                    canvas.drawBitmap(new_best, canw_mid-97, canh_mid-149, null);
                    if (new_index == 5) {
                        if (new_wait <= 5) {
                            canvas.drawBitmap(new_show, canw_mid-97, canh_mid-149, null);
                            new_wait++;
                        } else {
                            new_index = 0;
                            new_wait = 0;
                ...
                if( is_win )
                    canvas.drawBitmap(you_win, canw_mid-137, canh_mid-310, null);
                else if( !is_alive )
                    canvas.drawBitmap(game_over, canw_mid-127, canh_mid-273, null);

                if ( k < 10 )
                    canvas.drawBitmap(points[k], canw_mid-8, canh_mid-110, null);
                else if ( k < 100 ) {
                    canvas.drawBitmap(points[l], canw_mid-10, canh_mid-110, null);
                    canvas.drawBitmap(points[m], canw_mid+13, canh_mid-110, null);
                } else if ( k < 1000 ) {
                    canvas.drawBitmap(points[l], canw_mid-33, canh_mid-110, null);
                    canvas.drawBitmap(points[m], canw_mid-8, canh_mid-110, null);
                    canvas.drawBitmap(points[n], canw_mid+16, canh_mid-110, null);
                }else{
                    canvas.drawBitmap(points[l], canw_mid-44, canh_mid-110, null);
                    canvas.drawBitmap(points[m], canw_mid-20, canh_mid-110, null);
                    canvas.drawBitmap(points[n], canw_mid+3, canh_mid-110, null);
                    canvas.drawBitmap(points[p], canw_mid+27, canh_mid-110, null);
                }

                if ( k_old < 10 )
                    canvas.drawBitmap(points[k_old], canw_mid-8, canh_mid-14, null);
                else if ( k_old < 100 ) {
                    canvas.drawBitmap(points[l_old], canw_mid-10, canh_mid-14, null);
                    canvas.drawBitmap(points[m_old], canw_mid+13, canh_mid-14, null);
                } else if ( k_old < 1000 ) {
                    canvas.drawBitmap(points[l_old], canw_mid-33, canh_mid-14, null);
                    canvas.drawBitmap(points[m_old], canw_mid-8, canh_mid-14, null);
                    canvas.drawBitmap(points[n_old], canw_mid+16, canh_mid-14, null);
                }else{
                    canvas.drawBitmap(points[l_old], canw_mid-44, canh_mid-14, null);
                    canvas.drawBitmap(points[m_old], canw_mid-20, canh_mid-14, null);
                    canvas.drawBitmap(points[n_old], canw_mid+3, canh_mid-14, null);
                    canvas.drawBitmap(points[p_old], canw_mid+27, canh_mid-14, null);
                }
            }
            if ((((x4 < (x2 + 40)) && ((x4 + up_pipe.getWidth()) > x2) && (y2 < (y4 + up_pipe.getHeight() - 14))) ||
                    ((x5 < (x2 + 40)) && ((x5 + up_pipe.getWidth()) > x2) && (y2 < (y6 + up_pipe.getHeight() - 14))) ||
                    ((x4 < (x2 + 40)) && (((x4 + down_pipe.getWidth()) - 30) > x2) && ((y2 + 33) > y5)) ||
                    ((x5 < (x2 + 40)) && (((x5 + down_pipe.getWidth()) - 30) > x2) && ((y2 + 33) > y7)) ||
                    (y2 >= (canh-45))) && is_alive ) {
                hit.start();
                die.start();
                is_alive = false;
    ...
    public boolean onTouchEvent(MotionEvent event){
        ...
        if (x == MotionEvent.ACTION_DOWN) {
            if( initial )
                initial = false;
            else if ( (pressX < 48 && pressY < 48) && !paused ) {
                button.start();
                paused = true;
            }
            else if ( paused && pressX>circle_x && pressX<(circle_x+42) && pressY>circle_y && pressY<(circle_y+41) ){
                button.start();
                paused = false;
            }
            else if ( !is_alive && pressX>circle_x && pressX<(circle_x+42) && pressY>circle_y && pressY<(circle_y+41) ){
            ...
            else if( (!is_alive || paused ) && pressX>menu_x && pressX<(menu_x+100) && pressY>menu_y && pressY<(menu_y+50) ){
